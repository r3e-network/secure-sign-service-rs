...
pub extern "C" fn secure_sign_sgx_sign_extensible_payload(
    ...
    let payload = unsafe { slice::from_raw_parts(extensible_payload, extensible_payload_len) };
    ...
    let Ok(payload) = ExtensiblePayload::decode(payload) else {
    ...
    let script_hashes = script_hashes
        ...
        .collect();
    let signs = match signer.sign_extensible_payload(&payload, script_hashes, network) {
        Ok(r) => r,
        Err(err) => return sign_err_code(err),
    };
